...
synthetic_data <- read_csv("synthetic_data/synthetic_trajectories_ndvi_100sims_100steps.csv")
...
# synthetic_data_covs %>% filter(x2_cent < -buffer | x2_cent > buffer | y2_cent < -buffer | y2_cent > buffer)

# filter out when closer than 'buffer' to the edge
synthetic_data_covs2 <- synthetic_data_covs %>% filter(x2_ > 0 + buffer & x2_ < 2e4 - buffer & y2_ > 0 + buffer & y2_ < 2e4 - buffer) %>% drop_na(ta)

synthetic_data_covs2 <- synthetic_data_covs %>% filter(x2_cent > -buffer & x2_cent < buffer & y2_cent > -buffer & y2_cent < buffer) %>% drop_na(ta)
synthetic_data_covs2
...
write_csv(synthetic_data_df, paste0("synthetic_data_df_lag_", hourly_lag, "hr_n", n_samples, ".csv"))
...
rast(synthetic_data_covs$ndvi_cent) %>% 
  writeRaster(paste0("synthetic_ndvi_cent", nxn_cells, "x", nxn_cells, "_lag_", hourly_lag, "hr_n", n_samples, ".tif"), overwrite = T)
...
rast(synthetic_data_covs$pres_cent) %>% 
  writeRaster(paste0("synthetic_pres_cent", nxn_cells, "x", nxn_cells, "_lag_", hourly_lag, "hr_n", n_samples, ".tif"), overwrite = T)
...
synthetic_data_covs %>% ggplot() +
  geom_histogram(aes(x = sl), bins = 1000, alpha = 1) +
