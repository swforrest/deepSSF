...
def train_loop(dataloader_train, 
    ...
    # The raster_transform is not used in this function but we still need to unpack it.
    # We could also use an underscore _ to ignore it, but unpacking is clearer.
    for batch, (x1, x2, x3, y, raster_transform) in enumerate(dataloader_train):
        ...
        # print(x1.shape, x2.shape, x3.shape, y)
        ...
        if not skip_epoch0_training:
            ...
            # Uncomment if using CNN movement kernel 
...
def test_loop(dataloader_test, model, loss_fn):
    ...
    with torch.no_grad():
        ...
        for x1, x2, x3, y, raster_transform in dataloader_test:
