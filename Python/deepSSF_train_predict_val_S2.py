    ...
    def __init__(self, csv_file, transform=None, preload=True):
        ...
        if self.preload:
            ...
            for idx in tqdm(range(len(self.data))):
                ...
                # Sentinel-2 bands
                s2_b1_tens = torch.nan_to_num(torch.tensor(np.load(s2_b1_path), dtype=torch.float32), nan=0.0)
                s2_b2_tens = torch.nan_to_num(torch.tensor(np.load(s2_b2_path), dtype=torch.float32), nan=0.0)
                s2_b3_tens = torch.nan_to_num(torch.tensor(np.load(s2_b3_path), dtype=torch.float32), nan=0.0)
                s2_b4_tens = torch.nan_to_num(torch.tensor(np.load(s2_b4_path), dtype=torch.float32), nan=0.0)
                s2_b5_tens = torch.nan_to_num(torch.tensor(np.load(s2_b5_path), dtype=torch.float32), nan=0.0)
                s2_b6_tens = torch.nan_to_num(torch.tensor(np.load(s2_b6_path), dtype=torch.float32), nan=0.0)
                s2_b7_tens = torch.nan_to_num(torch.tensor(np.load(s2_b7_path), dtype=torch.float32), nan=0.0)
                s2_b8_tens = torch.nan_to_num(torch.tensor(np.load(s2_b8_path), dtype=torch.float32), nan=0.0)
                s2_b8a_tens = torch.nan_to_num(torch.tensor(np.load(s2_b8a_path), dtype=torch.float32), nan=0.0)
                s2_b9_tens = torch.nan_to_num(torch.tensor(np.load(s2_b9_path), dtype=torch.float32), nan=0.0)
                s2_b11_tens = torch.nan_to_num(torch.tensor(np.load(s2_b11_path), dtype=torch.float32), nan=0.0)
                s2_b12_tens = torch.nan_to_num(torch.tensor(np.load(s2_b12_path), dtype=torch.float32), nan=0.0)
                # Slope
                ...
                # Ensure the spatial data is a 4D tensor
...
csv_file = '/Users/scottforrest/deepSSF/buffalo_S2_all_steps_with_paths_n103558_steps.csv'
...
n_spatial_inputs = dataset_train[0][0].shape[0]
n_scalar_inputs = dataset_train[0][1].shape[0] # number of scalar inputs that are converted to a grid and appended to the spatial features
