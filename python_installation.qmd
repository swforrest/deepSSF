...
When you have Python, a package and environment manager, an IDE, and a working directory to work in (possibly linked to a GitHub repository), you are ready to start coding in Python. 

If you want to use the deepSSF code directly, then `forking` the [repo on GitHub](https://github.com/swforrest/deepSSF) is probably the easiest option, as we haven't turned this into a package (yet). This will create a linked copy of the repository in your own GitHub, which you can then clone onto your local computer, creating a working directory that you can work in. This means that you'll also have all of the code to create this website, which may not be relevant (but which may be helpful if you want to [make your own](https://quarto.org/docs/websites/) at some point). To navigate, all of the R coding scripts are in the root directory, and the Python scripts are in the `Python/` directory.
