...
tracking_data <- read_csv("data/SpaceCows/Mimal_processed.csv")
head(tracking_data)
...
tracking_data_clean <- tracking_data[, c(18, 1, 3, 4)]

# rename the columns
colnames(tracking_data_clean) <- c("id", "time", "lon", "lat")

# convert the time to the correct timezone
attr(tracking_data_clean$time, "tzone") <- "Australia/Queensland"
head(tracking_data_clean)

# check timezone
tz(tracking_data_clean$time)

# create a vector of the unique ids for indexing later
tracking_data_ids <- unique(tracking_data_clean$id)
...
tracking_data_all <- tracking_data_clean %>% mk_track(id = id,
                                          lon,
                                          lat, 
                                          time, 
                                          all_cols = T,
...
tracking_data_all_nested <- tracking_data_all %>% arrange(id) %>% nest(data = -"id")

tracking_data_all_nested_steps <- tracking_data_all_nested %>%
...
tracking_data_all_steps <- tracking_data_all_nested_steps %>% 
  amt::select(id, steps) %>% 
  amt::unnest(cols = steps)

head(tracking_data_all_steps)
...
prop_steps <- tracking_data_all_steps %>% filter(x2_ - x1_ > -distance & 
                                             x2_ - x1_ < distance &
                                             y2_ - y1_ > -distance & 
                                             y2_ - y1_ < distance) %>% 
  nrow() / tracking_data_all_steps %>% nrow()
...
tracking_data_all_steps_save <- tracking_data_all_steps %>% mutate(
...
tracking_data_all_steps_save

# remove steps that fall outside of the local spatial extent
tracking_data_all_steps_save <- tracking_data_all_steps_save %>%
...
tracking_data_all_steps_save %>% write_csv("data/tracking_data_all_steps.csv")
...
# for(i in 1:length(tracking_data_ids)) {
for(i in 1:1) { # select just the first ID

  tracking_data_data <- tracking_data_all %>% filter(id == tracking_data_ids[i])
  # all data for that individual
  tracking_data_data <- tracking_data_data %>% arrange(t_)
  
  # to use a subset of the data for that individual for testing
  # COMMENT THIS LINE OUT TO USE THE FULL DATASET
  # tracking_data_data <- tracking_data_data %>% arrange(t_) |>  slice(1:10)

  n_samples <- nrow(tracking_data_data)
  
  tic()
  
  tracking_data_data_covs <- tracking_data_data %>% mutate(
    ...
    pres_cent_x1 = list(rasterize(points_vect_cent_x1, pres_template, background=0)),
    ...
    points_vect_cent_x2 = list(terra::vect(cbind(x2_, y2_), type = "points", crs = "EPSG:3112")),
    
    pres_cent_x2 = list({
      pres_cent_x2 <- rasterize(points_vect_cent_x2, pres_template, background=0)
      ext(pres_cent_x2) <- extent_00centre
      pres_cent_x2
      })
  ...
  # remove steps that fall outside of the local spatial extent
  tracking_data_data_covs <- tracking_data_data_covs %>%
    filter(x2_cent > -buffer & x2_cent < buffer & y2_cent > -buffer & y2_cent < buffer) %>%
    drop_na(ta)

  # remove the columns that are no longer needed
  tracking_data_data_df <- tracking_data_data_covs %>%
  dplyr::select(
    -extent_00centre,
    -ndvi_cent,
    -veg_herby_cent,
  ...
  # save the data
  write_csv(tracking_data_data_df, paste0("tracking_data_local_data_id/website_temp/tracking_data_", tracking_data_ids[i],
                                    "_data_df_lag_", hourly_lag, "hr_n", n_samples, ".csv"))

  # saving the raster objects
  rast(tracking_data_data_covs$ndvi_cent) %>%
    writeRaster(paste0("tracking_data_local_layers_id/tracking_data_", tracking_data_ids[i], "_ndvi_cent",
                       nxn_cells, "x", nxn_cells, "_lag_", hourly_lag, "hr_n", n_samples, ".tif"),
                overwrite = T)

  rast(tracking_data_data_covs$veg_herby_cent) %>%
    writeRaster(paste0("tracking_data_local_layers_id/tracking_data_", tracking_data_ids[i], "_herby_cent",
                       nxn_cells, "x", nxn_cells, "_lag_", hourly_lag, "hr_n", n_samples, ".tif"),
                overwrite = T)

  rast(tracking_data_data_covs$canopy_cover_cent) %>%
    writeRaster(paste0("tracking_data_local_layers_id/tracking_data_", tracking_data_ids[i], "_canopy_cent",
                       nxn_cells, "x", nxn_cells, "_lag_", hourly_lag, "hr_n", n_samples, ".tif"),
                overwrite = T)

  rast(tracking_data_data_covs$slope_cent) %>%
    writeRaster(paste0("tracking_data_local_layers_id/tracking_data_", tracking_data_ids[i], "_slope_cent",
                       nxn_cells, "x", nxn_cells, "_lag_", hourly_lag, "hr_n", n_samples, ".tif"),
                overwrite = T)

  rast(tracking_data_data_covs$pres_cent_x2) %>%
    writeRaster(paste0("tracking_data_local_layers_id/fixed_tracking_data_", tracking_data_ids[i], "_pres_cent",
                       nxn_cells, "x", nxn_cells, "_lag_", hourly_lag, "hr_n", n_samples, ".tif"),
                overwrite = T)
...
head(tracking_data_data_df)
head(tracking_data_data_covs)
which(is.na(tracking_data_data_covs$ta))
...
for(i in 1:n_samples) {
  print(rowColFromCell(tracking_data_data_covs$pres_cent_x1[[i]], which(tracking_data_data_covs$pres_cent_x1[[i]][] == 1)))
}
...
# x2 uncentered
for(i in 1:n_samples) {
  print(rowColFromCell(tracking_data_data_covs$pres_cent_x2[[i]], which(tracking_data_data_covs$pres_cent_x2[[i]][] == 1)))
}
...
# NDVI
walk(tracking_data_data_covs$ndvi_cent[1:n_plots], terra::plot)

# Herbaceous vegetation
walk(tracking_data_data_covs$veg_herby_cent[1:n_plots], terra::plot)

# Canopy cover
walk(tracking_data_data_covs$canopy_cover_cent[1:n_plots], terra::plot)

# Slope
walk(tracking_data_data_covs$slope_cent[1:n_plots], terra::plot)

# Target (location of the next step)
walk(tracking_data_data_covs$pres_cent_x1[1:n_plots], terra::plot)

# Target (location of the next step)
walk(tracking_data_data_covs$pres_cent_x2[1:n_plots], terra::plot)
...
# saveRDS(tracking_data_data_covs, paste0("tracking_data_data_id_covs_", Sys.Date(), ".rds"))
...
#   terra::plot(tracking_data_data_covs$ndvi_cent[[i]])
#   dev.off()
# }
# 
# # for the pres layers
...
#   layer <- tracking_data_data_covs$pres_cent[[i]]
#   layer[layer == 0] <- 0.5
#   
#   ndvi_layer <- tracking_data_data_covs$ndvi_cent[[i]]
...
#       width = 150, height = 150, units = "mm", res = 600)
#   terra::plot(tracking_data_data_covs$veg_herby_cent[[i]])
#   dev.off()
# }
# 
# for(i in 1:n_plots) {
...
#       width = 150, height = 150, units = "mm", res = 600)
#   terra::plot(tracking_data_data_covs$canopy_cover_cent[[i]])
#   dev.off()
# }
# 
# for(i in 1:n_plots) {
...
#       width = 150, height = 150, units = "mm", res = 600)
#   terra::plot(tracking_data_data_covs$slope_cent[[i]])
#   dev.off()
# }
...
# plot step lengths
tracking_data_data_covs %>% ggplot() +
