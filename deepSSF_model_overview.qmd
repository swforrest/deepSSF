...
To use deep learning in this context, we can again imagine separating out the habitat selection and the movement processes. This allows us to replace both the log-linear habitat selection component and the estimation of the movement parameters with deep learning components.
...
There are two subnetworks: a habitat selection and a movement process subnetwork.
...
For the habitat selection process, we need to go from our spatial covariates to a habitat selection probability surface. We can do this using convolutional layers, which process spatial information and transform it into something else. 
