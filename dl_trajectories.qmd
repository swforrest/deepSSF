...
library(tidyverse)
packages <- c("amt", "sf", "terra", "beepr", "tictoc", "viridis", "scales", "ggpubr", "patchwork")
...
sim_data_filenames <- grep("3000", sim_data_full_list, value = T) %>% 
  grep("id2005", x = ., value = T) 

# to select a subset of the list of filenames for plotting
sim_data_all <- sim_data_filenames[1:5] %>% 
...
buffalo_hourly_habitat <-
  buffalo_id_subset %>% dplyr::group_by(hour, id) %>%
    summarise(n = n(),
