...
dl_trajectory <- read_csv("Python/outputs/dl_trajectories/dl_trajectory_10000steps_1.csv")
head(dl_trajectory)
...
buffalo_2014_1000samples <- buffalo_all %>% 
  filter(id == 2014) %>% 
  arrange(t_) |>  
  slice(1:100) |> 
  transmute(id = id, x = x_, y = y_, t = t_)

time_vector = buffalo_2014_1000samples$t
...
all_data <- bind_rows(buffalo_2014_1000samples, sim_data_all)

ggplot() +
  geom_path(data = sim_data_all,
            aes(x = x, y = y, colour = id)) +
  scale_colour_viridis_d() +
  geom_path(data = buffalo_2014_1000samples,
            aes(x = x, y = y), colour = "red") +
  coord_equal() +
  theme_bw()
