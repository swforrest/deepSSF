...
library(tidyverse)
packages <- c("amt", "sf", "terra", "beepr", "tictoc", "geostats", "viridis")
...
buffalo_id_1000samples <- buffalo_all %>% 
  filter(id == 2005) %>% 
  arrange(t_) |>  
  slice(1:1000) #|> 
  # transmute(id = id, x = x_, y = y_, t = t_, hour = hour)

time_vector = buffalo_id_1000samples$t
...
sim_data_full_list <- 
  list.files("Python/outputs/dl_trajectories/ta_mix_norm_jitter/", pattern = "*.csv", full.names = T)

sim_data_all <- grep("3000", sim_data_full_list, value = T) %>% 
...
# data
sim_data_all_subset <- sim_data_all %>% group_by(id) %>% slice_head(n = 1000) %>% ungroup()
...
plot_extent <- sim_data_all_subset %>%
...
ggplot() +
  ...
  geom_path(data = sim_data_all_subset,
...
ggsave("outputs/sim_data_all_ndvi.png", width = 150, height = 200, units = "mm", dpi = 600)
...
all_data <- bind_rows(buffalo_id_1000samples, sim_data_all)

ggplot() +
  geom_path(data = sim_data_all,
            aes(x = x, y = y, colour = id)) +
  scale_colour_viridis_d() +
  geom_path(data = buffalo_id_1000samples,
...
# dl_trajectory <- buffalo_id_1000samples
...
dl_track <- dl_trajectory %>% amt::mk_track(id = id, x, y, t1, all_cols = T, crs = 3112) %>% arrange(id)
...
buffalo_id_1000samples_covs <- buffalo_all %>% 
  filter(id == 2005) %>% 
  arrange(t_) |>  
  slice(1:1000)

ggplot() +
  geom_density(data = buffalo_id_1000samples_covs,
                 aes(x = sl),
...
ggsave("outputs/step_length.png", width = 100, height = 75, units = "mm", dpi = 600)
...
ggsave("outputs/step_length_log.png", width = 100, height = 75, units = "mm", dpi = 600)
...
ggplot() +
  ...
  geom_density(data = dl_trajectory,
                 aes(x = ta_corr1),
                 fill = "orange", colour = "black", alpha = 0.5) +
  scale_x_continuous("Turning angles (radians)") +
  scale_y_continuous("Density") +
  theme_bw()

ggsave("outputs/turning_angles.png", width = 150, height = 75, units = "mm", dpi = 600)
...
## Density of step length and turning angles for each hour
...
buffalo_id_1000samples_xy <- data.frame("x" = buffalo_id_1000samples$x_, "y" = buffalo_id_1000samples$y_)

buffalo_id_1000samples <- buffalo_id_1000samples |> mutate(
  ndvi_late_dry = terra::extract(ndvi_late_dry, buffalo_id_1000samples_xy)[,2],
  slope = extract(slope, buffalo_id_1000samples_xy)[,2],
  veg_herby = extract(veg_herby, buffalo_id_1000samples_xy)[,2],
  canopy_cover = extract(canopy_cover, buffalo_id_1000samples_xy)[,2]
)

buffalo_id_1000samples
...
buffalo_hourly_habitat <-
  buffalo_id_1000samples %>% dplyr::group_by(hour, id) %>%
    summarise(n = n(),
              step_length_mean = mean(sl),
              step_length_median = median(sl),
              step_length_sd = sd(sl),
              ndvi_late_dry_mean = mean(ndvi_late_dry),
              ndvi_late_dry_median = median(ndvi_late_dry),
              ndvi_late_dry_sd = sd(ndvi_late_dry),
