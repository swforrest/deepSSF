...
for(i in 1:length(buffalo_ids)) {
  ...
  buffalo_data <- buffalo_data %>% arrange(t_) |>  slice(4000:4010)
...
# NDVI
png("outputs/ndvi_local_env.png", width = 75, height = 60, units = "mm", res = 600)  # Adjust width, height, and resolution as needed
terra::plot(buffalo_data_covs$ndvi_cent[[5]], main = "NDVI")
dev.off()  # Close the graphic device to save the file

# Herby vegetation
png("outputs/herby_local_env.png", width = 75, height = 60, units = "mm", res = 600)  # Adjust width, height, and resolution as needed
terra::plot(buffalo_data_covs$veg_herby_cent[[5]], main = "Herbaceous veg")
dev.off()  # Close the graphic device to save the file

# Canopy cover
png("outputs/canopy_local_env.png", width = 75, height = 60, units = "mm", res = 600)  # Adjust width, height, and resolution as needed
terra::plot(buffalo_data_covs$canopy_cover_cent[[5]], main = "Canopy cover")
dev.off()  # Close the graphic device to save the file

# Slope
png("outputs/slope_local_env.png", width = 75, height = 60, units = "mm", res = 600)  # Adjust width, height, and resolution as needed
terra::plot(buffalo_data_covs$slope_cent[[5]], main = "Slope")
dev.off()  # Close the graphic device to save the file
...
ndvi_coef <- (buffalo_data_covs$ndvi_cent[[5]] * ndvi_beta) + (buffalo_data_covs$ndvi_cent[[5]]^2 * ndvi2_beta)
herby_coef <- buffalo_data_covs$veg_herby_cent[[5]] * herby_beta
canopy_coef <- buffalo_data_covs$canopy_cover_cent[[5]] * canopy_beta
slope_coef <- buffalo_data_covs$slope_cent[[5]] * slope_beta

# NDVI
png("outputs/ndvi_local_env_beta.png", width = 75, height = 60, units = "mm", res = 600)  # Adjust width, height, and resolution as needed
terra::plot(ndvi_coef, main = "NDVI")
dev.off()  # Close the graphic device to save the file

# Herby vegetation
png("outputs/herby_local_env_beta.png", width = 75, height = 60, units = "mm", res = 600)  # Adjust width, height, and resolution as needed
terra::plot(herby_coef, main = "Herbaceous veg")
dev.off()  # Close the graphic device to save the file

# Canopy cover
png("outputs/canopy_local_env_beta.png", width = 75, height = 60, units = "mm", res = 600)  # Adjust width, height, and resolution as needed
terra::plot(canopy_coef, main = "Canopy cover")
dev.off()  # Close the graphic device to save the file

# Slope
png("outputs/slope_local_env_beta.png", width = 75, height = 60, units = "mm", res = 600)  # Adjust width, height, and resolution as needed
terra::plot(slope_coef, main = "Slope")
dev.off()  # Close the graphic device to save the file

rsf <- exp(ndvi_coef + herby_coef + canopy_coef + slope_coef)

# RSF
png("outputs/rsf_local_env.png", width = 90, height = 90, units = "mm", res = 600)  # Adjust width, height, and resolution as needed
terra::plot(rsf, main = "RSF")
dev.off()  # Close the graphic device to save the file
...
res <- 25     # Resolution in meters

# Create an empty SpatRaster with specified dimensions and resolution
r <- rast(nrows = nrow, ncols = ncol, xmin = 0, xmax = ncol * res,
          ymin = 0, ymax = nrow * res)

# Define the reference point (center of the raster)
x0 <- (xmin(r) + xmax(r)) / 2
y0 <- (ymin(r) + ymax(r)) / 2

# Get the coordinates of each cell center
coords <- crds(r, df = TRUE)

# Calculate distances from each cell to the reference point
dx <- coords$x - x0
dy <- coords$y - y0
distances <- sqrt(dx^2 + dy^2)

# Calculate bearings from the reference point to each cell (in degrees from North)
bearings_rad <- atan2(dx, dy)  # Note: atan2(x, y)
bearings_deg <- (bearings_rad * 180 / pi + 360) %% 360

# Convert bearings to circular data suitable for the von Mises distribution
bearings_circular <- circular(bearings_deg, units = "degrees", template = "geographics")

# Define parameters for the gamma distribution
gamma_shape <- 1    # Shape parameter
gamma_scale <- 300   # Scale parameter

# Define parameters for the von Mises distribution
mean_direction <- circular(90, units = "degrees", template = "geographics")  # Mean direction at 90 degrees (East)
kappa <- 0.25  # Concentration parameter

# Calculate the gamma density for each distance
gamma_densities <- dgamma(distances, shape = gamma_shape, scale = gamma_scale)

# Calculate the von Mises density for each bearing
vonmises_densities <- dvonmises(bearings_circular, mu = mean_direction, kappa = kappa)

# Compute the joint density by multiplying the gamma and von Mises densities
joint_densities <- gamma_densities * vonmises_densities

# Assign the joint densities to the raster cells
values(r) <- joint_densities

# Plot the resulting density raster
png("outputs/move_local_env.png", width = 90, height = 90, units = "mm", res = 600)
plot(r, 
     # main = "Movement Distribution",
     xlab = "x", ylab = "y"
     )
dev.off()  
...
# Plot the next-step density raster
png("outputs/step_local_env.png", width = 90, height = 90, units = "mm", res = 600)
plot(r * rsf)
dev.off()  
