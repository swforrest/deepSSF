...
library(tidyverse)
packages <- c("amt", "sf", "terra", "beepr", "tictoc", "circular", "matrixStats")
...
ssf_coefs <- read_csv("ssf_coefficients/TwoStep_2pDaily_coefs_dry_2024-09-05.csv")
...
test_data <- data_id2005 %>% slice(1:1000)
...
ggplot() +
  geom_point(data = test_data, 
             aes(x = hour_t2, y = prob_next_step_ssf, fill = yday_t2), 
             alpha = 0.1) +
