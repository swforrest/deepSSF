...
# set.seed(12321)
# ndvi_dry <- grf(ncell(r), grid="reg", nx=50, ny=50, xlims=c(0,50), ylims=c(0,50), cov.pars=c(1, 10))
...
# ndvi_dry_raster <- rast(raster(ndvi_dry))
# plot(ndvi_dry_raster)
# names(ndvi_dry_raster) <- "ndvi_dry"
# 
# hist(values(ndvi_dry_raster), breaks = 30)
...
# simulated_locs_fit_list <- lapply(1:10, function(i) {
#   simulate_ssf(n_steps = 1000,
#                n_ch = 50,
#                gamma_shape = gamma_shape,
#                gamma_scale = gamma_scale,
#                coef = ssf_coef,
#                resc = ndvi_dry_raster,
#                xy0 = c(runif(1, 0, 50), runif(1, 0, 50)))
# })

# simulated_locs_fit <- bind_rows(simulated_locs_fit_list, .id = "sim")
...
lapply(simulated_locs_test_list, function(df) {
  df$t = ymd_hm("2019-01-01 00:00") + hours(1:nrow(df))
  return(df)
  })

simulated_locs_test <- bind_rows(simulated_locs_test_list, .id = "sim")
...
write_csv(simulated_locs_test, paste0("synthetic_data/synthetic_trajectories_ndvi_", n_sims, "sims_", n_steps, "steps.csv"))
