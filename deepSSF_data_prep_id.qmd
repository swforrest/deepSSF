...
for(i in 1:1) { # select just the first ID
  ...
  #               overwrite = T)
  # 
  # rast(buffalo_data_covs$pres_cent_x2) %>%
  #   writeRaster(paste0("buffalo_local_layers_id/fixed_buffalo_", buffalo_ids[i], "_pres_cent",
  #                      nxn_cells, "x", nxn_cells, "_lag_", hourly_lag, "hr_n", n_samples, ".tif"),
  #               overwrite = T)
...
# x2 uncentered
for(i in 1:n_samples) {
  print(rowColFromCell(buffalo_data_covs$pres_cent_x2[[i]], which(buffalo_data_covs$pres_cent_x2[[i]][] == 1)))
}
...
# Target (location of the next step)
walk(buffalo_data_covs$pres_cent_x2[1:n_plots], terra::plot)
