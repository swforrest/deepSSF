---
title: "Assessing deepSSF trajectories against observed data"
...
for(i in 0:23) {
  print(ggplot() +
    geom_density(data = buffalo_id_subset_covs |> dplyr::filter(hour_t2 == i),
           aes(x = sl),
...
for(i in 0:23) {
  print(ggplot() +
    geom_density(data = buffalo_id_subset_covs |> filter(hour_t2 == i),
           aes(x = ta),
           fill = "skyblue", colour = "black", alpha = 0.5) +
    geom_density(data = dl_trajectory |> filter(hour == i),
                   aes(x = ta_corr1),
...
dl_trajectory <- dl_trajectory %>%
  mutate(
    ...
    ndvi = sapply(c(x, y), function(s) terra::extract(ndvi_projected[[ndvi_index]], s)[,2])
  )
...
dl_trajectory <- dl_trajectory %>%
  mutate(
    ndvi = terra::extract(ndvi_projected[[ndvi_index]], c(x, y))[,2],
...
buffalo_id_subset <- buffalo_id_subset %>% mutate(
  ndvi = terra::extract(ndvi_projected[[ndvi_index]], buffalo_id_subset_xy)[,2],
...
dl_hourly_habitat <- 
  dl_trajectory %>% dplyr::group_by(hour_t2, id) %>% 
    summarise(n = n(),
              step_length_mean = mean(sl, na.rm = TRUE),
              step_length_median = median(sl, na.rm = TRUE),
              step_length_sd = sd(sl, na.rm = TRUE),
