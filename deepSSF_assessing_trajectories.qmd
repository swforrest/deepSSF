...
ggplot() +
  ...
  geom_path(data = sim_data_all %>% filter(id %in% 1:10),
...
buffalo_id_subset_covs <- buffalo %>% 
  filter(id == 2005) %>% 
  arrange(t_) |>  
  slice(1:no_subset_locations)
...
# vapply(dl_trajectory$t1, function(t) which.min(abs(difftime(t, terra::time(ndvi)))), integer(1))
# quantile(dl_trajectory$t1)

hist(dl_trajectory$ndvi_index)

ndvi_values_sim <- terra::extract(ndvi[[ndvi_index]], dl_trajectory_xy)[,2]
...
buffalo_id_subset <- buffalo_id_subset %>%
  mutate(
    ndvi_index = vapply(t_, function(t) which.min(abs(difftime(t, terra::time(ndvi)))), integer(1))
  )

hist(buffalo_id_subset$ndvi_index)

ndvi_values_buffalo <- terra::extract(ndvi[[ndvi_index]], buffalo_id_subset_xy)[,2]
