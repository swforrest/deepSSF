...
no_subset_locations <- 1000 # to compare to the simulated data
...
all_data <- all_data %>%
  mutate(ndvi_index = vapply(t1, 
                             function(t) which.min(abs(difftime(t, terra::time(ndvi)))), 
                             integer(1)))

# ndvi_values <- map2_dbl(
#   all_data$ndvi_index,
#   seq_len(nrow(all_data_xy)),
#   function(index, row_num) {
#     # Extract the value from the 'idx'-th layer at the coordinate of row row_num
#     terra::extract(ndvi[[index]], all_data_xy[row_num, , drop = FALSE])[[2]]
#   }
# )
...
tic()
for(i in 1:nrow(all_data_xy)){
  ndvi_values[i] <- terra::extract(ndvi[[all_data$ndvi_index[i]]], all_data_xy[i,])[[2]]
}
toc()

# # extract ndvi values from the correct layer
# ndvi_values_sim <- terra::extract(ndvi[[ndvi_index]], all_data_xy)[,2]
# 
# all_data %>% 
#   rowwise() %>% 
#   mutate(ndvi = terra::extract(ndvi[[ndvi_index]], c(x1, y1))[,2])
...
all_data_hourly_habitat <-
  ...
  mutate(Data = ifelse(id == 2005, "Observed", "Simulated"))

# write.csv(buffalo_hourly_habitat,
#           paste0("outputs/buffalo_summaries_hourly_habitat_", Sys.Date(), ".csv"))

all_data_hourly_habitat_long <- all_data_hourly_habitat %>%
...
hist(sim_data_all$yday)

dl_daily_habitat <- sim_data_all %>% 
  dplyr::group_by(yday, id) %>% 
  summarise(n = n(),
